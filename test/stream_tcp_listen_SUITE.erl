-module(stream_tcp_listen_SUITE).
...
mk_listener(IP, Port, TmpDir) ->
    ...
    libp2p_stream_tcp_listen_sup:start_link(IP, Port,
                                            #{cache_dir => TmpDir,
...
listen_port0_test(Config) ->
    ...
    ListenAddrs = libp2p_stream_tcp_listen_sup:listen_addrs(Pid),
...
reuse_port0_test(Config) ->
    ...
    [{_, Port} | _] = libp2p_stream_tcp_listen_sup:listen_addrs(Pid),
    exit(Pid, normal),
    ?assert(test_util:pid_should_die(Pid)),

    TmpDir = ?config(cache_dir, Config),
    {ok, NewPid} = mk_listener(TmpDir),
    [{_, NewPort} | _] = libp2p_stream_tcp_listen_sup:listen_addrs(NewPid),
...
listen_ip_test(Config) ->
    ...
    [IPAddr | _] = libp2p_stream_tcp_listen_socket:get_non_local_addrs(),

    %% Make a listener and find out the address an dport
    {ok, Pid} = mk_listener(IPAddr, TmpDir),
    [{ListenAddr, ListenPort} | _] = libp2p_stream_tcp_listen_sup:listen_addrs(Pid),
    ...
    {ok, NewPid} = mk_listener(IPAddr, TmpDir),
    %% And verify the port was reused
    ?assertMatch([{ListenAddr, ListenPort} | _ ], libp2p_stream_tcp_listen_sup:listen_addrs(NewPid)),
...
listen_port_test(Config) ->
    ...
    %% Make a listener and find out the address an dport
    {ok, Pid} = mk_listener(IPAddr, Port, TmpDir),
    [{ListenAddr, ListenPort} | _] = libp2p_stream_tcp_listen_sup:listen_addrs(Pid),
    ?assertEqual({IPAddr, Port}, {ListenAddr, ListenPort}),

    %% Restart the listener with the same cache and ip address
    {ok, NewPid} = mk_listener(IPAddr, Port, TmpDir),
    %% And verify the port was reused
    ?assertMatch([{ListenAddr, ListenPort} | _ ], libp2p_stream_tcp_listen_sup:listen_addrs(NewPid)),
