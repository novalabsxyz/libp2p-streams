-module(libp2p_stream_tcp_listen_sup).
...
-spec start_link(inet:ip_address(), inet:port_number(), Opts :: map()) ->
    {ok, pid()} | {error, term()}.
...
listen_addrs(Sup) ->
    ...
    libp2p_stream_tcp_listen_socket:listen_addrs(Pid).
...
init([_IP, _Port, Opts] = ListenArgs) ->
    ...
    Pool = #{
        id => ?POOL_ID,
        start => {libp2p_stream_tcp_accept_pool, start_link, [Opts]}
    },
    Socket = #{
        id => ?LISTEN_SOCKET_ID,
        start => {libp2p_stream_tcp_listen_socket, start_link, [self() | ListenArgs]}
    },
