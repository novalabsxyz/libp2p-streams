...
init(
    ...
    libp2p_stream_muxer:identify(Muxer, #{
        identify_keys => IdentKeys,
        ...
        <<ExPubKey:32/binary, Signature/binary>> ->
            ...
            case libp2p_crypto:verify(ExPubKey, Signature, RemotePubKey) of
                ...
                true ->
                    ...
                    handle_init_result(Result, State#state{
                        exchanged = true,
                        send_key = SendKey,
                        rcv_key = RcvKey
                    })
    ...
    case decrypt_data(Data, State#state.rcv_key, Nonce) of
        ...
        {ok, Bin} ->
            ...
            case libp2p_packet:decode_packet(State#state.packet_spec, Bin) of
                ...
                {more, _N} ->
                    lager:error("Decrypted packet not complete for spec ~p", [
                        State#state.packet_spec
                    ]),
                    {stop, normal, State}
            end
    end.
...
handle_info(
    _Kind,
    {handle_identify, stream_secure_identify, {ok, Identify}},
    State = #state{
...
handle_actions([{send, Data} | Tail], {State = #state{send_nonce = Nonce}, Acc}) ->
    ...
    handle_actions(
        Tail,
        {State#state{send_nonce = Nonce + 1}, [{send, encode_data(EncryptedData)} | Acc]}
    );
