...
init({Sup, IP, Port, Opts=#{cache_dir := CacheDir}}) ->
    ...
    Filename = string:replace(string:slice(libp2p_stream_tcp:to_multiaddr({IP, Port}), 1) ++ ".cache",
                              "/", "_", all),
    ...
    case gen_tcp:listen(ReusedPort, ListenOptions) of
        {ok, Socket} ->
            ...
            dets:insert(CacheName, {{listen_addrs, libp2p_stream_tcp:ip_addr_type(IP)}, ListenAddrs}),
            dets:close(CacheName),
            % acceptor could close the socket if there is a problem
            MRef = monitor(port, Socket),
            MaxInboundConnections =maps:get(max_inbound_connections, Opts,
...
handle_info({register_socket, Sup, MaxInboundConnections}, State=#state{}) ->
    ...
    {noreply, State};
handle_info({'DOWN', MRef, port, Socket, Reason}, State=#state{socket=Socket, socket_monitor=MRef}) ->
    {stop, Reason, State};
handle_info(_, State) ->
    {noreply, State}.
...
listen_options(IP) ->
    InetOpts = case libp2p_stream_tcp:ip_addr_type(IP) of
...
listen_addresses(Socket, NatMap) ->
    ...
    case lists:all(fun(D) -> D == 0 end, tuple_to_list(IP)) of
        ...
        true ->
            ...
            [apply_nat_map({Addr, Port}, NatMap)
