...
-type identify_map() :: #{ pubkey_bin => libp2p_crypto:pubkey_bin(),
                           observed_addr => string(),
                           nonce => binary()
                         }.
-export_type([identify/0, identify_map/0]).
-export([from_map/2, encode/1, decode/1, verify/1,
         pubkey_bin/1, observed_maddr/1, observed_addr/1, nonce/1]).
...
from_map(#{ pubkey_bin := PubKeyBin,
            observed_addr := ObservedAddr,
            nonce := Nonce},
         SigFun) ->
    Identify = #libp2p_identify_pb{pubkey=PubKeyBin,
